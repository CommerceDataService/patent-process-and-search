...
// Only doing this for readiblity. Do not accept blank or undefined dates
  ...
  {
    var s = (req.query.pageno -1) *20;
    var currentPage = parseInt(req.query.pageno) ;
  }
  ...
  if (!req.query.pageno)
  {
   var SEARCH_URL= 'http://54.208.116.77:8983/solr/oafiledatanew/select?q='+q+'&wt=json&indent=true&rows=20&start=0&hl=true&hl.snippets=10&hl.fl=textdata&hl.fragsize=200&hl.simple.pre=<code>&hl.simple.post=</code>&hl.usePhraseHighlighter=true&q.op=AND&fl=appid,action_type,filename,minread,id,textdata'; 
   var currentPage = 1;
  }
  else
  {
    var SEARCH_URL= 'http://54.208.116.77:8983/solr/oafiledatanew/select?q='+q+'&wt=json&indent=true&rows=20&start='+s+'&hl=true&hl.snippets=10&hl.fl=textdata&hl.fragsize=200&hl.simple.pre=<code>&hl.simple.post=</code>&hl.usePhraseHighlighter=true&q.op=AND&fl=appid,action_type,filename,minread,id,textdata';
  }
...
if (!req.query.pageno)
{
  var SEARCH_URL= 'http://54.208.116.77:8983/solr/oafiledatanew/select?q='+q+'&wt=csv&indent=falue&rows=100&start=0&hl=false&hl.snippets=5&hl.fl=textdata&hl.fragsize=200&hl.simple.pre=<code>&hl.simple.post=</code>&hl.usePhraseHighlighter=true&q.op=AND&fl=appid,action_type,filename,minread,id,textdata';
  var currentPage = 1;
}
else
{
  var SEARCH_URL= 'http://54.208.116.77:8983/solr/oafiledatanew/select?q='+q+'&wt=csv&indent=true&rows=20&start='+s+'&hl=true&hl.snippets=5&hl.fl=textdata&hl.fragsize=200&hl.simple.pre=<code>&hl.simple.post=</code>&hl.usePhraseHighlighter=true&q.op=AND&fl=appid,action_type,filename,minread,id,textdata';
}
